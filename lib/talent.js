...
(function(global){
  ...
  checkAndHandleTalentLoss(student) {
    ...
    registerDefaultTalents(game, utils){
      ...
      this.registerTalent({
        name: '胆怯',
        description: '在比赛中容易紧张导致所有能力临时-20%，压力越高越容易触发',
        color: '#4C4C4C',
        prob: 0.15,
        beneficial: false,
        handler: function(student, eventName, ctx){
          try{
            ...
            // 触发概率基础 50%，赛前压力每 10 点额外 +5%
            let base = 0.50;
            const pressure = Number(student.pressure) || 0;
            base += Math.floor(pressure / 10) * 0.05;
            if(getRandom() < base){
              // 使用临时增益系统：基础值 × 20%
              const thinkingBonus = student._base_thinking * -0.20;
              const codingBonus = student._base_coding * -0.20;
              const mentalBonus = student._base_mental * -0.20;
              ...
              return '胆怯发动：全能力 -20%（赛中临时）';
            }
          }catch(e){ console.error('胆怯 天赋错误', e); }
          return null;
        }
      });
      ...
      this.registerTalent({
        ...
        handler: function(student, eventName, ctx){
          try{
            ...
            if(getRandom() < 0.20){
              // 使用临时增益系统
              const thinkingBonus = -student._base_thinking * 0.10;
              
              student.applyTempModifier('thinking', student.getTempModifier('thinking') + thinkingBonus);
              
              return '焦虑发动：思维能力临时-10%（本题）。';
            }
          }catch(e){ console.error('焦虑 天赋错误', e); }
          return null;
        }
      });
      this.registerTalent({
        name: '内耗',
        description: '跳题后有一定概率使得各方面能力小幅下降',
        color: '#3E3E3E',
        prob: 0.05,
        beneficial: false,
        handler: function(student, eventName, ctx){
          try{
            ...
            if(eventName === 'contest_select_problem'){
              ...
              if(cnt >= 1 && getRandom() < 0.3333){
                // 使用临时增益系统
                const thinkingBonus = -0.10 * student._base_thinking;
                const dsBonus =  -0.10 *student.knowledge_ds;
                const graphBonus =  -0.10 *student.knowledge_graph;
                const stringBonus =  -0.10 *student.knowledge_string;
                const mathBonus =  -0.10 *student.knowledge_math;
                const dpBonus =  -0.10 *student.knowledge_dp;
                ...
                return '内耗发动：跳题导致各方面能力小幅下降（本题临时）';
              }
            }
            // reset on solve
          ...
          }catch(e){ console.error('内耗 天赋错误', e); }
          return null;
        }
      });
      ...
      this.registerTalent({
        name: '慢热',
        description: '比赛中解题数达到 2 题后，心理素质提升 20%。',
        color: '#2196F3',
        prob: 0.10,
        beneficial: true,
        handler: function(student, eventName, ctx){
          try{
            ...
            if(solvedCount >= 2){
              student._talent_state = student._talent_state || {};
              if(!student._talent_state['慢热_triggered']){
                const mentalBonus = student._base_mental * 0.20;
                student.applyTempModifier('mental', student.getTempModifier('mental') + mentalBonus);
                student.applyTempModifier('constmental', student.getTempModifier('constmental') + mentalBonus);
                student._talent_state['慢热_triggered'] = true;
                return '慢热发动：心理素质 +20%（赛中临时）';
              }
            }
      ...
      this.registerTalent({
        name: '天助',
        description: '思考阶段有概率在当前题目直接得满分，压力越大概率越高',
        color: '#9C27B0',
        prob: 0.03,
        beneficial: true,
        handler: function(student, eventName, ctx){
          try{
            ...
            // 最高有 25% 可能直接跳
            if(getRandom() < 0.25 * (Number(student.pressure) || 0)){
              const state = ctx && ctx.state;
              const pid = ctx && (ctx.problemId || ctx.problemId === 0 ? ctx.problemId : ctx.problemId);
              if(!state) return null;
              const probObj = state.getProblem(pid);
              ...
              if(probObj.solved){
                return `天助发动：直接 AC 了 T${pid+1}（满分 ${lastSub.score}）`;
              }
              return `天助发动：直接通过了 T${pid+1} 的最后一档，得分 ${lastSub.score}`;
            }
          }catch(e){ console.error('天助 天赋错误', e); }
          return null;
        }
      });
      ...
      this.registerTalent({
        ...
        this.registerTalent({
          name: '手生',
          description: "若选中题标签含 '数据结构'，在比赛和模拟赛中临时使 数据结构能力 减半。",
          color: '#002C54',
          prob: 0.08,
          beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              // 使用临时增益系统
              const dsBonus = student.knowledge_ds * -0.50;
              student.applyTempModifier('knowledge_ds', student.getTempModifier('knowledge_ds') + dsBonus);
              
              return '手生发动：本题 数据结构能力 减半（本题临时）';
            }catch(e){ console.error('手生 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '波动',
          description: "在模拟赛选题有概率使得思维 +10% 或 -10%。",
          color: '#3F51B5',
          prob: 0.05,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(getRandom() < 0.50){
                // 使用临时增益系统
               const dir = (getRandom() < 0.50 ? 1 : -1);
                const thinkingBonus = student._base_thinking * 0.10 * dir;
                student.applyTempModifier('thinking', student.getTempModifier('thinking') + thinkingBonus);
            
                return '波动发动：思维 ' + (dir > 0 ? '+' : '-') + '10%（本题临时）';
              }
            }catch(e){ console.error('波动直觉 天赋错误', e); }
            return null;
          }
        });
            ...
            try{
              ...
              if(eventName === 'contest_start' || eventName === 'mock_start'){
                if(state && Array.isArray(state.problems)){
                  student._talent_state._狂热_half = Math.ceil(state.problems.length / 2);
                  student._talent_state._狂热_appliedFirst = false;
                  student._talent_state._狂热_appliedSecond = false;
                }
                return null;
              }
              ...
              if(eventName === 'contest_select_problem'){
                ...
                if(pid < half && !student._talent_state._赛场狂热_appliedFirst){
                  ...
                  student._talent_state._狂热_appliedFirst = true;
                  return '狂热（前半段）发动：思维 +25%（赛中临时）';
                }
                ...
                if(pid >= half && !student._talent_state._赛场狂热_appliedSecond){
                  ...
                  return '狂热（后半段）发动：心理素质下降至 80%（赛中临时）';
                }
              }
            }catch(e){ console.error('狂热 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '轻敌',
          description: '压力值过低时，赛时所有知识下降 25%',
             color: '#3A3A3A',
          prob: 0.30,
          beneficial: false,
          handler: function(student, eventName, ctx){
        ...
        this.registerTalent({
          name: '急躁',
          description: '跳题概率上升，且跳一次题思维 -10%（上限 5 层）',
          color: '#009688',
          prob: 0.08,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              const newLayers = Math.min(5, layers + 1);
              student._talent_state._jump_layers = newLayers;
              // Remove old modifier and apply new one: each层 +10% 叠加
              const oldBonus = student._base_thinking * (Math.pow(0.9, layers) - 1);
              const newBonus = student._base_thinking * (Math.pow(0.9, newLayers) - 1);
              student.applyTempModifier('thinking', student.getTempModifier('thinking') - oldBonus + newBonus);
              student._talent_state['急躁'] = true;
              return `急躁发动：跳题次数增加至 ${newLayers} 层，思维 -${(newLayers*10)}%（赛中临时）`;
            }catch(e){ console.error('急躁 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '偏科',
          description: '比赛中偏科：随机将一个知识点 +50%，另一个 -50%。',
          color: '#673AB7',
          prob: 0.04,
          beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              student.applyTempModifier(up, student.getTempModifier(up) + baseUp * 0.50); // +200%
              student.applyTempModifier(down, student.getTempModifier(down) - baseDown * 0.50); // -50%
              student._talent_state['偏科'] = { up: up, down: down };
              return `偏科发动：${up.replace('knowledge_','')} +50%，${down.replace('knowledge_','')} -50%（本场临时）`;
        ...
        this.registerTalent({
          name: '松懈',
          description: '训练强度小于一定值有概率不增加压力，但使知识增益减少50%',
        color: '#607D8B',
        prob: 0.15,
        kind: 'negative',
        beneficial: true,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(getRandom() < 0.5){
                return { action: 'moyu_cancel_pressure', reduceKnowledgeRatio: 0.50, message: '松懈：本次训练压力取消，知识增益-50%'};
              }
            }catch(e){ console.error('松懈 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '敏感',
          description: '当压力增加不足 15 时自动让增幅变为 15。',
        color: '#009688',
        prob: 0.12,
        kind: 'negative',
        beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(amt < 15){
                return { action: 'add_15_pressure', message: '敏感：本次压力增幅为 15' };
              }
            }catch(e){ console.error('敏感 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '遗忘',
          description: '放假结束时，有概率让所有知识点小幅下降。',
        color: '#3F51B5',
        prob: 0.07,
        kind: 'negative',
        beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(getRandom() < 0.50){
                if(typeof student['knowledge_ds'] === 'number'){
                  student['knowledge_ds'] -= 3;
                }
                if(typeof student['knowledge_graph'] === 'number'){
                  student['knowledge_graph'] -= 3;
                }
                if(typeof student['knowledge_string'] === 'number'){
                  student['knowledge_string'] -= 3;
                }
                if(typeof student['knowledge_math'] === 'number'){
                  student['knowledge_math'] -= 3;
                }
                if(typeof student['knowledge_dp'] === 'number'){
                  student['knowledge_dp'] -= 3;
                }
                return '遗忘：所有知识点小幅下降';
              }
            }catch(e){ console.error('遗忘 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '怕生',
          description: '外出集训，不熟悉的环境有概率使得压力增加',
        color: '#795548',
        prob: 0.05,
        kind: 'negative',
        beneficial: false,
          handler: function(student, eventName, ctx){
             try{
              ...
              if(!prov) return null;
              if(getRandom() <= 0.50){
                return { action: 'double_pressure', message: '怕生：本次集训压力翻倍' };
              }
            }catch(e){ console.error('怕生 天赋错误', e); }
            return null;
          }
        });
        ...
        this.registerTalent({
          name: '沉溺',
          description: '打游戏时如果压力过低，将直接退队去学电竞。',
        color: '#F57C00',
        prob: 0.02,
        kind: 'negative',
        beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              if(Number(student.pressure || 0) <= 20){
                student.active = false;
                student._quit_for_esports = true;
                return '沉溺：直接退队去学电竞';
              }
            }catch(e){ console.error('沉溺 天赋错误', e); }
            return null;
          }
        });
        ...
        beneficial: false,
          handler: function(student, eventName, ctx){
            try{
                ...
                return { action: 'get_sick', message: '体弱：获得生病状态' };
              }catch(e){ console.error('体弱 天赋错误', e); }
            return null;
          }
        });
        ...
        beneficial: false,
          handler: function(student, eventName, ctx){
            try{
              ...
              student.pressure += 20;
              return'风恐：压力增加';
            }catch(e){ console.error('风恐 天赋错误', e); }
            return null;
          }
        });
